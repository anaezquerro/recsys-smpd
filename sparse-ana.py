...
class Sparse:
    ...
    def preprocess(self):
        ...
        self.save_sparse(relations, self.train_path, remove_zerorow=True)
        self.save_sparse(test_relations, self.test_path, remove_zerorow=True,
                         rowmap_path=self.test_path.replace('.npz', '.pickle'))
        end = time.time()
    ...
    def save_sparse(self, relations: Dict[int, Tuple[int]], path: str, rowmap_path: str = None,
        ...
        if remove_zerorow:
            ...
            assert all([len(tracks) > 0 for tracks in relations.values()])
